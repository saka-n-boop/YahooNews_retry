...
# 修正: YouTubeTranscriptApiクラスとエラーを標準的にインポート
...
def get_transcript(video_id: str) -> Optional[str]:
    ...
    try:
        # 修正: YouTubeTranscriptApiのインスタンスを作成し、
        # 存在が確認されているfetchメソッドを使用してトランスクリプトを取得します。
        # fetch()はget_transcript()と同じ構造のデータを返します。
        transcript_list = YouTubeTranscriptApi().fetch(video_id, languages=['ja', 'en'])
